 ...
 *  Если установить значение равно 0 то функция отключится. И возможность полива будет проверяться
 *  исходя из показаний текущего времени и переменной WATERING_INTERVAL.
 */
int PUMP_LIMIT = 10000;

/*  WATERING_INTERVAL - минимальный интервал (в часах), который будет выдержан перед повторным поливом растения. При установленном значении
...
void checkConditions() {
  ...
  if ( watering_pass_time >= WATERING_INTERVAL
       && current_hour > GOOD_MORNING && current_hour < GOOD_NIGHT) {
        ...
        if(wetness > WET_LIMIT || WET_LIMIT == 0){
          action();
